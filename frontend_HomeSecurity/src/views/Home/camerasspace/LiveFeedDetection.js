...
const LiveFeedDetectionrtsp = () => {
  ...
  return (
    <div className="p-6 bg-moon-light text-moon-dark rounded-lg shadow-lg">
      ...
      <p>
    This component displays a live video feed from a camera using RTSP and WebRTC technologies. 
    The camera streams video through RTSP, which is processed by a YOLO object detection model hosted in the cloud. 
    The YOLO processor analyzes the RTSP stream (e.g., from 'camera1'), detects objects or people, 
    and sends the processed video to a smart mode RTSP stream (e.g., 'camera1smart').
    The smart mode RTSP feed communicates with the WebRTC server, allowing the detected video to be displayed on this web page in real-time.
    When "Smart Mode" is enabled, you'll see the processed video with YOLO detections applied to the feed.
  </p>
      </Modal>
        ...
        {selectedCameraID && (
          <div className="mt-6 space-y-4">
            ...
            {isCameraOpen && currentCamera ? (
              <MqttVideoReceiver
                brokerUrl={settings.processingbrokerURL}
                topic={`camera/${selectedCameraID}/${smartMode ? 'detectionframe' : 'simpleframe'}`} // Use smart mode to determine topic
              ...
              <p className="text-gray-500">Select a camera and open it to view the live feed.</p>
            )}
          </div>
        )}
      </div>
